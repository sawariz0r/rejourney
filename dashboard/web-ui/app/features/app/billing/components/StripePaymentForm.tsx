...
import { addPaymentMethod, createSetupIntent } from '~/features/app/billing/api';
import { NeoButton } from '~/shared/ui/core/neo/NeoButton';
import { CreditCard, AlertTriangle, Check, X, Loader2 } from 'lucide-react';
import { getStripeClient, getStripeKeyStatus } from '~/shared/integrations/stripe';
...
export function StripePaymentForm({ teamId, onSuccess, onCancel }: PaymentFormProps) {
  ...
  const [error, setError] = useState<string | null>(null);
  const stripeKeyStatus = getStripeKeyStatus();

  useEffect(() => {
    if (stripeKeyStatus === 'missing') {
      if (import.meta.env.DEV) {
        console.warn('[Stripe] No publishable key configured');
      }
    }

    if (stripeKeyStatus !== 'valid') {
      setIsLoading(false);
      return;
    }
