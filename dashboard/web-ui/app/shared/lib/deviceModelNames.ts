...
const DEVICE_MODEL_NAMES_BY_KEY = new Map(
    Object.entries(DEVICE_MODEL_NAMES).map(([identifier, name]) => [identifier.toLowerCase(), name]),
);

const GENERIC_IDENTIFIER_LABELS: Array<[RegExp, string]> = [
    [/^iphone\d+,\d+$/i, 'iPhone'],
    [/^ipad\d+,\d+$/i, 'iPad'],
    [/^ipod\d+,\d+$/i, 'iPod touch'],
    [/^watch\d+,\d+$/i, 'Apple Watch'],
    [/^appletv\d+,\d+$/i, 'Apple TV'],
    [/^audioaccessory\d+,\d+$/i, 'HomePod'],
...
export const formatDeviceModel = (
    ...
    if (!raw || raw.toLowerCase() === 'unknown' || raw.toLowerCase() === 'unknown device' || raw.toLowerCase() === 'n/a') {
        return fallback;
    }
...
export const getDeviceModelSearchText = (model: string | null | undefined): string => {
    const raw = (model || '').trim();
    const formatted = formatDeviceModel(raw, '');
    return `${raw} ${formatted}`.trim().toLowerCase();
};
