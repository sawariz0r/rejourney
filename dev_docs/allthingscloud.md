...
- `trafficDistribution: PreferClose` — kube-proxy routes to the local node's pgbouncer when available, and automatically falls back to any pgbouncer replica when local is absent. Replaces the previous `internalTrafficPolicy: Local` which gave no fallback — during rolling updates the node briefly had no local pgbouncer and `Local` dropped all connections from that node instead of routing elsewhere (fixed 2026-04-26). Requires k8s 1.31+ (GA); cluster is on 1.33.
- Pod anti-affinity: `required` on hostname — exactly one pgbouncer per node in all states. With 3 nodes and 3 replicas this is a strict invariant. Do not change to `preferred` — the rolling update strategy relies on `maxUnavailable: 1` (not a surge pod), so required anti-affinity is correct.
- Rolling update: `maxSurge: 0, maxUnavailable: 1` — one pod at a time is replaced. There is no 4th node for a surge pod; a `maxSurge: 1` strategy causes the surge pod to stay Pending forever because existing running pods' `required` anti-affinity blocks co-location even on newly-scheduled pods. `PreferClose` handles the brief gap during the one-pod-down window.
