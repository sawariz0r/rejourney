...
The Revenue impact Custom events source uses the same `logEvent` payload. Use a real money-collected event such as `purchase_completed`; do not map setup or device events such as `device_info`, `app_initialized`, or page-view events as revenue.
