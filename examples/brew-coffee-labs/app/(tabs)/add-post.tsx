...
const COFFEE_BROWN = "#6F4E37"; // Coffee brown color for consistency
...
const AddPostScreen = () => {
  ...
  // Reset the modal visibility whenever the screen comes into focus
  useFocusEffect(
    useCallback(() => {
      console.log("AddPostScreen: Screen focused, showing welcome modal");
      setShowWelcomeModal(true);

      // Optional: Reset other form state when revisiting the screen
      setTitle('');
      setImageUri(null);
      setImageWidth(null);
      setImageHeight(null);
      ...
      return () => {
        ...
        console.log("AddPostScreen: Screen unfocused");
      };
    }, [])
  );

  // Handle user selecting to use Coffee Labs
  const handleUseCoffeeLabs = () => {
    console.log("AddPostScreen: User selected Coffee Labs option");
    setShowWelcomeModal(false);
    router.push('/other-pages/coffee-scanner');
  };

  // Handle user selecting to write manually
  const handleWriteManually = () => {
    console.log("AddPostScreen: User selected to write manually");
    setShowWelcomeModal(false);
  };
  ...
  const processImageResult = async (pickerResult: ImagePicker.ImagePickerResult) => {
    ...
    if (pickerResult.canceled) {
      console.log("AddPostScreen: Image picking/capture was cancelled.");
      return;
    }
    if (!pickerResult.assets || pickerResult.assets.length === 0 || !pickerResult.assets[0].uri) {
      console.warn("AddPostScreen: Image picker/camera result invalid.", pickerResult);
    ...
    console.log(`AddPostScreen: Image selected/captured. Temp Original URI: ${tempOriginalUri}, Picker Dimensions: ${pickedAsset.width}x${pickedAsset.height}`);
    console.log("AddPostScreen: Setting isLoading = true (before image processing)");
    setIsLoading(true);
    setLoadingMessage("Processing image...");

    // Use Image.getSize to get reliable dimensions, especially after camera capture
    Image.getSize(
      tempOriginalUri,
      async (origWidth, origHeight) => {
        ...
        if (!(origWidth > 0 && origHeight > 0)) {
          ...
          Alert.alert("Image Error", "Could not get valid image dimensions after selection/capture.");
        ...
        try {
          ...
          // Fallback to original for preview if manipulation fails
      ...
      (error) => {
        ...
        Alert.alert("Image Error", "Could not read image dimensions after selection/capture. The file might be corrupted or unsupported.");
        setImageUri(null); setImageWidth(null); setImageHeight(null);
        setOriginalImageUri(null); setOriginalImageWidth(null); setOriginalImageHeight(null);
  ...
  const pickImage = useCallback(async () => {
    ...
    // Reset image state before picking/taking new photo
    console.log("AddPostScreen: Resetting ALL image states before picking/taking photo.");
    setImageUri(null); setImageWidth(null); setImageHeight(null);
    setOriginalImageUri(null); setOriginalImageWidth(null); setOriginalImageHeight(null);
        ...
        [
          {
            text: "Cancel",
            style: "cancel",
            onPress: () => console.log("AddPostScreen: Image source selection cancelled."),
          },
          {
            text: "Take Photo...",
            onPress: takePhoto,
    ...
    const takePhoto = async () => {
      console.log("AddPostScreen: 'Take Photo' option selected.");
      try {
        console.log("AddPostScreen: Requesting camera permissions...");
        const cameraPermissionResult = await ImagePicker.requestCameraPermissionsAsync();
        console.log(`AddPostScreen: Camera permission status: ${cameraPermissionResult.status}`);

        if (cameraPermissionResult.status !== 'granted') {
          Alert.alert('Permission Denied', 'Access to your camera is needed to take a photo.');
          return;
        }

        console.log("AddPostScreen: Launching camera...");
        const result = await ImagePicker.launchCameraAsync({
          quality: 0.8, // Keep quality consistent
          allowsEditing: Platform.OS === 'android', // Allow editing on Android
          aspect: Platform.OS === 'android' ? [3, 4] : undefined, // Maintain aspect ratio if editing
          base64: false, // Don't need base64 here
        });
        console.log("AddPostScreen: Camera result received.");
        await processImageResult(result); // Process the result

      } catch (error) {
        console.error("AddPostScreen: Error taking photo:", error);
        Alert.alert("Camera Error", "An unexpected error occurred while trying to use the camera.");
        // Ensure loading state is reset in case of error during launch/permission
        if (isLoading) {
            setIsLoading(false);
            setLoadingMessage("Publishing...");
        }
    ...
    const chooseFromLibrary = async () => {
      console.log("AddPostScreen: 'Choose from Library' option selected.");
      try {
        if (Platform.OS !== 'web') {
          ...
          console.log(`AddPostScreen: Media library permission status: ${libraryPermissionResult.status}`);
          if (libraryPermissionResult.status !== 'granted') {
        ...
        console.error("AddPostScreen: Error choosing from library:", error);
        Alert.alert("Library Error", "An unexpected error occurred while selecting from the library.");
         // Ensure loading state is reset in case of error during launch/permission
         if (isLoading) {
            setIsLoading(false);
            setLoadingMessage("Publishing...");
  ...
  const handleBack = () => {
      if (!isLoading)         router.replace('/(tabs)/home');
  ...
  return (
    ...
    >
      ...
      >
        <BlurView intensity={20} style={styles.welcomeModalContainer} tint="dark">
          <View style={styles.welcomeModal}>
            <Text style={styles.welcomeTitle}>Create Coffee Recipe</Text>
            <Text style={styles.welcomeSubtitle}>How would you like to proceed?</Text>
            ...
            <View style={styles.optionsContainer}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={handleUseCoffeeLabs}
              >
                <LinearGradient
                  colors={['#8B5A2B', '#6F4E37', '#513A26']}
                  style={styles.gradientBackground}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
                >
                  <View style={styles.optionIconContainer}>
                    <Ionicons name="flask-outline" size={36} color="#FFF" />
                  </View>
                  <Text style={styles.optionTitle}>Coffee Labs</Text>
                  <Text style={styles.optionDescription}>
                    Scan ingredients and discover creative coffee recipes
                  </Text>
                  <View style={styles.optionButton}>
                    <Text style={styles.optionButtonText}>Coffee Labs</Text>
              ...
              >
                <View style={styles.manualContent}>
                  <View style={styles.optionIconContainer}>
                    <Ionicons name="create" size={34} color={COFFEE_BROWN} />
                  </View>
                  <Text style={[styles.optionTitle, styles.manualTitle]}>Write Recipe</Text>
                  <Text style={[styles.optionDescription, styles.manualDescription]}>
                    Create and share your own coffee recipe manually
                  </Text>
                  <View style={styles.manualButton}>
                    <Text style={styles.manualButtonText}>Write Manually</Text>
                    <Ionicons name="arrow-forward" size={18} color={COFFEE_BROWN} style={{marginLeft: 5}} />
                  </View>
                </View>
              </TouchableOpacity>
            </View>
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={pickImage} // This now shows the Alert with options
          disabled={isLoading}
        >
          {imageUri && imageWidth && imageHeight ? (
            <>
            ...
            <View style={styles.imagePickerContent}>
              ...
              <Text style={styles.imagePickerSubText}>(Tap to Choose or Take)</Text>
            </View>
          )}
