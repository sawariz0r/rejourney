...
struct CountryDetails: View {
    ...
    var body: some View {
        content
            ...
            .onAppear {
                RejourneyExample.trackScreen("Country Details")
                RejourneyExample.logEvent("country_details_viewed", properties: [
...
private extension CountryDetails {
    ...
    func loadCountryDetails(forceReload: Bool) {
        RejourneyExample.logEvent("country_details_load_requested", properties: [
            "alpha3_code": country.alpha3Code,
    ...
    func showCountryDetailsSheet() {
        RejourneyExample.logEvent("country_flag_opened", properties: [
            "alpha3_code": country.alpha3Code,
            "country_name": country.name(locale: locale)
        ])
        ...
        Section(header: Text("Neighboring countries")) {
            ForEach(neighbors) { country in
                ...
                .simultaneousGesture(TapGesture().onEnded {
                    RejourneyExample.logEvent("neighbor_country_selected", properties: [
                        "alpha3_code": country.alpha3Code,
                        "country_name": country.name(locale: self.locale)
                    ])
                })
            }
        }
