...
@objc(RJNativeEventBuffer)
final class EventBuffer: NSObject {
    
    @objc static let shared = EventBuffer()
    ...
    @objc var eventCount: Int {
        _lock.lock()
        defer { _lock.unlock() }
        return _eventCount
    }
    
    @objc var lastEventTimestamp: Int64 {
        _lock.lock()
        defer { _lock.unlock() }
    ...
    @objc func configure(sessionId: String) {
        _lock.lock()
        defer { _lock.unlock() }
    ...
    @objc func appendEvent(_ event: [String: Any]) -> Bool {
        _lock.lock()
        defer { _lock.unlock() }
    ...
    @objc func readPendingEvents() -> [[String: Any]] {
        _lock.lock()
        defer { _lock.unlock() }
    ...
    @objc func getPendingSessions() -> [String] {
        guard let cacheDir = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first else { return [] }
    ...
    @objc func readEventsForSession(_ sessionId: String) -> [[String: Any]] {
        guard let cacheDir = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first else { return [] }
