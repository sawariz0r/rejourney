...
@objc(RJNativeObjCRuntimeUtils)
final class ObjCRuntimeUtils: NSObject {
    
    @objc static func hotswap(cls: AnyClass, original: Selector, replacement: Selector) {
        guard let m1 = class_getInstanceMethod(cls, original),
              let m2 = class_getInstanceMethod(cls, replacement) else { return }
        method_exchangeImplementations(m1, m2)
    }
    
    @objc static func hotswapSafely(cls: AnyClass, original: Selector, replacement: Selector) {
        guard let m1 = class_getInstanceMethod(cls, original),
              let m2 = class_getInstanceMethod(cls, replacement) else { return }
