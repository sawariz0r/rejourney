...
async function ensureBackupSchema() {
  if (!(await tableExists('session_backup_log'))) {
    await sql(`
      CREATE TABLE session_backup_log (
  ...
  const sessionBackupLogColumns = [
    ['planned_artifact_count', 'ALTER TABLE session_backup_log ADD COLUMN planned_artifact_count INT NOT NULL DEFAULT 0;'],
    ['high_quality', 'ALTER TABLE session_backup_log ADD COLUMN high_quality BOOLEAN;'],
    ['quality_tier', 'ALTER TABLE session_backup_log ADD COLUMN quality_tier VARCHAR(32);'],
    ['quality_reason', 'ALTER TABLE session_backup_log ADD COLUMN quality_reason JSONB;'],
    ['quality_checked_at', 'ALTER TABLE session_backup_log ADD COLUMN quality_checked_at TIMESTAMPTZ;'],
    ['quality_rule_version', 'ALTER TABLE session_backup_log ADD COLUMN quality_rule_version INT;'],
    ['actual_r2_artifact_count', 'ALTER TABLE session_backup_log ADD COLUMN actual_r2_artifact_count INT;'],
    ['actual_r2_object_count', 'ALTER TABLE session_backup_log ADD COLUMN actual_r2_object_count INT;'],
    ['manifest_present', 'ALTER TABLE session_backup_log ADD COLUMN manifest_present BOOLEAN;'],
  ];
  for (const [columnName, ddl] of sessionBackupLogColumns) {
    if (!(await columnExists('session_backup_log', columnName))) {
      await sql(ddl);
    }
  }

  if (!(await indexExists('session_backup_log_backed_up_at_idx'))) {
    await sql('CREATE INDEX session_backup_log_backed_up_at_idx ON session_backup_log(backed_up_at);');
  }

  if (!(await tableExists('session_backup_queue'))) {
    await sql(`
      CREATE TABLE session_backup_queue (
        session_id VARCHAR(64) PRIMARY KEY,
  ...
  if (!(await indexExists('session_backup_queue_claim_idx'))) {
    await sql('CREATE INDEX session_backup_queue_claim_idx ON session_backup_queue(status, next_retry_at, created_at, session_id);');
  }
  if (!(await indexExists('session_backup_queue_stale_idx'))) {
    await sql('CREATE INDEX session_backup_queue_stale_idx ON session_backup_queue(status, claimed_at);');
  }

  if (!(await tableExists('session_backup_run_lock'))) {
    await sql(`
      CREATE TABLE session_backup_run_lock (
...
function buildNotFullyBackedUpPredicate(logAlias = 'bl', readyArtifactCountExpr = 'artifact_stats.ready_artifact_count') {
  ...
  return `
        (
          ${logAlias}.session_id IS NULL
          OR ${logAlias}.artifact_count < ${readyArtifactCountExpr}
          OR ${logAlias}.planned_artifact_count < ${readyArtifactCountExpr}
        )
  `.trim();
}
...
async function normalizePendingQueueRetryTimes() {
  const result = await sql(`
    UPDATE session_backup_queue
    SET
...
async function main() {
  ...
  try {
    const normalizedPendingRetryTimes = await normalizePendingQueueRetryTimes();
    if (normalizedPendingRetryTimes > 0) {
      log(`Normalized next_retry_at for ${normalizedPendingRetryTimes} queued session(s)`);
    }
